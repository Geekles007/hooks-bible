...
type Action<T> =
    | { type: 'loading'; }
    | { type: 'init'; payload: (data?: Object) => void }
    | { type: 'fetched'; payload: T }
    | { type: 'error'; payload: Error }

function useMutation<T = unknown>(url?: string, options?: RequestInit): State<T> {
    const cache = useRef<Cache<T>>({});
    ...
    const fetchReducer = (state: State<T>, action: Action<T>): State<T> => {
        switch (action.type) {
            case 'init':
                return { ...initialState, execute: action.payload }
            case 'loading':
                return { ...initialState, loading: true }
            case 'fetched':
    ...
    useEffect(() => {
        ...
        dispatch({type: "init", payload: execute});
...
export default useMutation;
            ...
            description: <p>
                ...
                request.
                <br/>
                You can also execute the data by calling the <strong>execute</strong> function.
            </p>
        }
        ],
        [
